...
/**
 * Get all hirings (with filtering) 
 * GET /api/hiring
 * 
 */
...
exports.createHiring = async (req, res) => {
  ...
  try {
    ...
    const hiringNumber =
      "HIR-" + crypto.randomBytes(4).toString("hex").toUpperCase();

    // Create a temporary hiring object to use the model's calculation method
    const tempHiring = new Hiring({
      route,
      bus, // Add bus reference for route-based calculation
      startDate,
      endDate,
      tripType: tripType || (isRoundTrip ? 'Round-Trip' : 'One-Way'),
      rateType: rateType || 'Fixed', // Default to Fixed if not provided  
      baseRate,
      routePriceMultiplier,
      estimatedDistance,
      additionalCharges,
    ...
    // Calculate total cost using the model's method
    let calculatedTotalCost;
    try {
      calculatedTotalCost = await tempHiring.calculateTotalCost();
    } catch (error) {
      return res.status(400).json({
        status: "error",
    ...
    // Validate provided totalCost if given
    if (totalCost && Math.abs(totalCost - calculatedTotalCost) > 0.01) {
      return res.status(400).json({
        status: "error",
        message: `Provided totalCost (${totalCost}) does not match calculated cost (${calculatedTotalCost})`,
    ...
    const newHiring = new Hiring({
      ...
      additionalCharges,
      driverAllowance,
      totalCost: totalCost || calculatedTotalCost,
      cancellationPolicy,
 ...
 * @access  Private (own hiring or admin)
 */
exports.processHiringPayment = async (req, res) => {
  try {
    ...
    // Check if the user is allowed to make payment for this hiring
    const isAdmin = req.user.role === 'admin';
    const isOwner = hiring.user.toString() === req.user.id.toString();
    ...
    if (!isAdmin && !isOwner) {
      return res.status(403).json({
        status: 'error',
        message: 'You are not authorized to make payment for this hiring request'
      });
    }
    ...
    // Check if the payment amount doesn't exceed remaining balance
    const remainingBalance = hiring.totalCost - hiring.totalPaid;
    if (amount > remainingBalance + 0.01) { // Allow for small rounding differences
      return res.status(400).json({
        status: 'error',
    ...
    const result = await hiring.processPayment({
      ...
      transactionId: transactionId || reference || `PAY-${crypto.randomBytes(4).toString('hex').toUpperCase()}`
    });
    
    // Send automatic payment success notification with payment details
    try {
      const notificationService = new NotificationService(req.io);
      await notificationService.sendBookingNotification(hiring, 'payment_successful', {
        amount,
        method: method || 'Paystack',
        transactionId: transactionId || reference || `PAY-${crypto.randomBytes(4).toString('hex').toUpperCase()}`,
        currency: '₦',
    ...
    console.error('Error processing hiring payment:', error);
    res.status(500).json({
      status: 'error',
...
exports.deleteHiring = async (req, res) => {
  try {
    ...
    // Store hiring details for response
    const hiringDetails = {
      id: hiring._id,
      hiringNumber: hiring.hiringNumber,
      status: hiring.status
    ...
    // Permanently delete the hiring record
    ...
    res.status(500).json({
      status: 'error',
      message: 'Server error while deleting hiring request',
      error: process.env.NODE_ENV === 'development' ? error.message : undefined
    });
  }
};
...
/**
 * @desc    Generate hiring ticket/receipt
 * @route   GET /api/hiring/:id/receipt
 * @access  Private (own hiring or admin)
 */
exports.getHiringReceipt = async (req, res) => {
  try {
    const hiring = await Hiring.findById(req.params.id)
      .populate('user', 'name email phone')
      .populate('bus', 'busNumber type capacity amenities');
    ...
    if (hiring.paymentStatus !== 'Paid') {
      return res.status(400).json({
        status: 'error',
        message: 'Cannot generate ticket for unpaid hiring'
      });
    }

    // Generate ticket with QR code
    const ticket = await generateTicket(hiring, 'hiring');

    res.status(200).json({
      status: 'success',
      message: 'Hiring ticket generated successfully',
      data: {
        ticket,
        hiring: {
          id: hiring._id,
    ...
    console.error('Error generating hiring ticket:', error);
    res.status(500).json({
      status: 'error',
      message: 'Server error while generating hiring ticket',
      error: process.env.NODE_ENV === 'development' ? error.message : undefined
    });
  }
};
 ...
 * @access  Private (own hiring or admin)
 */
exports.downloadHiringTicketPDF = async (req, res) => {
  try {
    const hiring = await Hiring.findById(req.params.id)
      .populate('user', 'name email phone')
      .populate('bus', 'busNumber type capacity amenities');
    ...
    if (hiring.paymentStatus !== 'Paid') {
      return res.status(400).json({
        status: 'error',
        message: 'Cannot generate ticket PDF for unpaid hiring'
      });
    }
    ...
    res.setHeader('Content-Disposition', `attachment; filename="hiring-ticket-${hiring.hiringNumber}.pdf"`);
    res.setHeader('Content-Length', pdfBuffer.length);

    res.send(pdfBuffer);
  } catch (error) {
    console.error('Error generating hiring ticket PDF:', error);
    res.status(500).json({
      status: 'error',
      message: 'Server error while generating hiring ticket PDF',
      error: process.env.NODE_ENV === 'development' ? error.message : undefined
    });
  }
};
