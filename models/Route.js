...
const RouteSchema = new mongoose.Schema(
  {
    ...
    departureTime: {
      ...
      match: [
        ...
        "Departure time must be in HH:MM format (24-hour)",
      ],
    },
    arrivalTime: {
      type: String,
      ...
      validate: {
        validator: function (value) {
          if (this.departureTime && this.estimatedDuration) {
            const [depHours, depMinutes] = this.departureTime
              .split(":")
              .map(Number);
            const [arrHours, arrMinutes] = value.split(":").map(Number);
            const depTotalMinutes = depHours * 60 + depMinutes;
            const arrTotalMinutes = arrHours * 60 + arrMinutes;
            const durationMinutes = this.estimatedDuration;
...
RouteSchema.methods.calculateFare = function(options = {}) {
  ...
  if (date) {
    ...
    isPeakTimeValue = isPeakTimeValue || ((hours >= 7 && hours <= 10) || (hours >= 16 && hours <= 19));
    isWeekendValue = isWeekendValue || (day === 0 || day === 6);
  }
...
RouteSchema.methods.generateSchedule = async function(startDate, endDate) {
  ...
  while (currentDate <= end) {
    ...
    if (this.operatingDays.includes(dayOfWeek)) {
      ...
      let [arrHours, arrMinutes] = this.arrivalTime.split(":").map(Number);

      // Handle midnight-crossing routes for arrival time
      let arrivalDatePart = datePart;
      if (arrHours * 60 + arrMinutes < depHours * 60 + depMinutes) {
        // Arrival is on the next day
        const arrivalDate = new Date(currentDate);
        arrivalDate.setDate(currentDate.getDate() + 1);
        arrivalDatePart = arrivalDate.toISOString().split("T")[0];
      }
      ...
      schedule.push({
        ...
        fullDepartureDateTime: `${datePart}T${this.departureTime}:00Z`, // For API consumers
